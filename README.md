...
**Historical, already done, do not repeat**: PR #9's merge called for running `state_rm` against `restapi_object.oauth_app["all_apps"], restapi_object.oauth_app["vm_apps"]` before the next `plan-and-apply`, on the assumption that the failing run had never gotten past `terraform plan` and so had created nothing. That assumption turned out to be wrong: a first run had already created both OAuth apps, the rotate resources, and both org OIDC configs live before a second, stale run executed this exact `state_rm` against them. Running this `state_rm` again now would remove two entries that are live and back in state (see the `state_import` section immediately below); do not run it for these two addresses again.

### Escape hatch: `terraform state import` via workflow_dispatch (recovering from a bad state_rm)

`configure-private-cloud.yml`'s `workflow_dispatch` trigger also has an optional `state_import` input, symmetric to `state_rm` and run strictly after it: a space or comma separated list of `addr=id` pairs, each run as `terraform import -var-file="envs/lab.tfvars" -- "<addr>" "<id>"` before planning (the `-var-file` matters: `restapi_object.oauth_app` is keyed off `var.orgs`, and `orgs`/`vcfa_url`/`sso_realm_id` only come from `envs/lab.tfvars`, which isn't auto-loaded). This exists for the failure mode `state_rm` itself can cause: a stale run executes `state_rm` against a resource a different, newer run already created live, and a subsequent apply then tries to CREATE an object that already exists (VCFA IAM's oauth-apps endpoint 400s on duplicate `clientName`, "display name exists"). The right fix there is not "recreate it", the OAuth app is live and is already the `client_id` the org's `vcfa_org_oidc` config authenticates with; recreating would mint a different object and break that federation. The fix is putting the existing live object back into Terraform state via import.
...
- **Redirect URL** (`"%s/tenant/%s/oauth/callback"`): the `/oauth/callback` suffix is a **guess**, not confirmed against a live OIDC client registration. `vcfa_org_oidc` computes its own true redirect URI as an exported attribute (`oidc_redirect_uri`, surfaced by `module.orgs`) only *after* the org's OIDC federation is created, which happens strictly after this OAuth app resource on first apply, so the real value can't be known at the point this payload is built. After a first apply, compare `module.orgs[*].oidc_redirect_uri` against what got registered here; if they differ, the OAuth app's `redirectUrls` needs a follow-up fix. `restapi_object.oauth_app` (`orgs.tf`) ignores changes to `data`, since the Ops IAM oauth-apps API has no update endpoint, so overriding `var.oauth_app_redirect_url_pattern` alone produces a clean, empty plan without correcting the URL actually registered on the server: it looks fixed and isn't. The override still needs to happen (so the next replace registers the right value), but the registered app itself only changes via a targeted replace: `terraform apply -replace='restapi_object.oauth_app["<key>"]'`, which recreates the client and mints a new `clientId`/`clientSecret` (module.orgs' OIDC config picks it up automatically), or a manual patch by hand in the Ops portal if a client identity change isn't acceptable.
